...
function useFadecordDonorBadges(userID: string) {
    const [FadecordDonorBadges, setFadecordDonorBadges] = useState<[string, string][]>([]);

    useEffect(() => {
        fetch("https://raw.githubusercontent.com/Fadecord/Ignore/main/badges.csv")
            .then(res => res.text())
            .then(data => {
                const badges = [...data.matchAll(new RegExp(`(${userID}),([^,\n]+),([^,\n]+)`, "g"))];
                if (badges === null) return;
                setFadecordDonorBadges(badges.map(m => [m[2], m[3]]));
            })
            .catch(e => { console.error(e); });
    }, []);

    return FadecordDonorBadges;
}

function FadecordDonorChatBadges(userID: string) {
    const FadecordDonorBadges = useFadecordDonorBadges(userID);

    return FadecordDonorBadges.length > 0 ? [
        <span style={{ order: settings.store.FadecordDonorBadgesPosition }}>
            {FadecordDonorBadges.map(badge =>
                <RoleIconComponent
                    className={roleIconClassName}
                    name={badge[0]}
                    size={20}
...
function vencordFadecordContributorChatBadge(userID: string) {
    return isPluginDev(userID) ? [
        <span style={{ order: settings.store.vencordFadecordContributorBadgePosition }}>
            <RoleIconComponent
                className={roleIconClassName}
                name={"Fadecord Plugin Helpers"}
                size={20}
                src={"https://www.fadecord.online/Fadecord_files/logo-nav-oneko-padding.png"}
            />
        </span>
    ] : [];
}
...
function ChatBadges({ author }) {
    const chatBadges = [
        ...
        ...settings.store.showFadecordDonorBadges ? FadecordDonorChatBadges(author.id) : [],
        ...settings.store.showVencordFadecordContributorBadges ? vencordFadecordContributorChatBadge(author.id) : [],
...
const settings = definePluginSettings({
    ...
    showFadecordDonorBadges: {
        type: OptionType.BOOLEAN,
        description: "Enable to show Fadecord donor badges in chat.",
        default: true
    },
    FadecordDonorBadgesPosition: {
        type: OptionType.NUMBER,
        description: "The position of the Fadecord Donor badges.",
        default: 0
    },
    showVencordFadecordContributorBadges: {
        type: OptionType.BOOLEAN,
        description: "Enable to show Vencord/Fadecord contributor badges in chat.",
        default: true
    },
    vencordFadecordContributorBadgePosition: {
        type: OptionType.NUMBER,
        description: "The position of the Vencord/Fadecord Contributor badge.",
        default: 1
    },
